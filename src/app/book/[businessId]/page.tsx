...
async function getBusinessData(businessId: string): Promise<{ business: WithId<Business>, services: WithId<Service>[] } | null> {
    try {
        const { firestore } = initializeFirebase();
        
        // Fetch business
        const businessRef = doc(firestore, "businesses", businessId);
        const businessSnap = await (await import('firebase/firestore')).getDoc(businessRef);
        ...
        const services = servicesSnap.docs.map(doc => ({ id: doc.id, ...doc.data() } as WithId<Service>));

        return { business, services };

    } catch (error) {
        console.error("Failed to fetch business data on server:", error);
        return null;
    }
}
...
export default async function BusinessPublicPage({ params }: { params: { businessId: string } }) {
    const data = await getBusinessData(params.businessId);
                     ...
                     <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                        ...
                        {services?.map((service) => (
                            <Card key={service.id} className="flex flex-col">
                                ...
                                <CardFooter>
                                    <Link href={`/book/${business.id}/service/${service.id}`} className="w-full">
